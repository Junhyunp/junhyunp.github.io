...
## 2026-08-16 — 취재원 스윕 + 리드 검증 (CHOI / threads)

닷새간 클라우드 루틴이 0건이라 **로컬에서 직접 훑은** 회차입니다. 조회 자체는 정상이었습니다 — 원인은 취재원이 아니라 클라우드 egress 차단이고, 상세는 `sources.yml` 상단에 적었습니다.

### 조회 결과

| 취재원 | 결과 |
|---|---|
| CHOI (threads `@choi.openai`) | ✅ 글 8건 조회. 팔로워 280,000 (08-12 대비 +14,000) |

⚠️ **`curl` 로는 0건이 나옵니다.** 256KB HTML 이 오지만 계정명·한글·본문 필드가 전부 없는 JS 껍데기입니다. `WebFetch` 로는 정상입니다. curl 결과만 보고 "안 열린다"고 판단하면 오진입니다.

### 리드 4건 → 1차 출처 검증

| 리드 | 1차 출처 | 판정 |
|---|---|---|
| a16z 뉴클라우드 리포트 | [a16z.news](https://www.a16z.news/p/charts-of-the-week-head-in-the-neoclouds) (Sternstein, 2026-08-14) | ✅ **글감 승격 → 발행**(`neocloud-economics`) |
| Liquid AI LFM2.5-Encoder PII | [HF 모델 카드](https://huggingface.co/LiquidAI/LFM2.5-Encoder-350M-PII-Detector) | ✅ 사실. 다만 인코더는 2026-07-29 공개라 새 소식이 아님. 보류 |
| Cursor 인수 | TipRanks / SeekingAlpha 등 | ✅ 사실 — SpaceX가 Anysphere를 **$60B 전액 주식**, 2026-06-16 발표 / **2026-08-14 완료**. 공식 발표문은 못 찾음 |
| Dario Amodei / SB 53 | [SB 53 법안 원문](https://leginfo.legislature.ca.gov/faces/billTextClient.xhtml?bill_id=202520260SB53) | ⚠️ **보류** — $500M 기준은 법안 원문에서 확인(`in excess of five hundred million dollars`). 그러나 CHOI 가 가리킨 글 자체는 X 게시물이라 **HTTP 402 로 접근 불가** |

### 2차 출처와 원문이 어긋난 지점

- **capex $7.1B** — CHOI 가 적었으나 **a16z 원문에 없음.** 별도 검색의 Q2 $9.4B / 연간 $35~39B 과도 불일치 → **글에서 뺐음**
- **Cursor "어제 90조"** — 실제 $60B(≈84조, 환율 1,400원 기준), 완료일도 08-14 로 하루 어긋남
- 25분기/40분기, 감가상각 50% 초과, 인프라 전환 계보는 **원문과 일치**

### 인용 거부

X(트위터) 게시물은 `WebFetch` 가 **HTTP 402** 를 반환해 1차 확인이 불가능합니다. X 를 유일 출처로 하는 건은 **쓰지 않습니다** — 2차 보도(CNBC·Axios·ANI)로 대체하면 이 블로그의 검증 기준을 못 지킵니다.

### 교훈

**SNS 의 수치는 방향만 맞고 자릿수가 틀릴 수 있다.** 이번엔 인수가·날짜·capex 세 군데가 어긋났는데, 전부 "대충 맞아 보이는" 수준이라 원문을 안 열었으면 그대로 나갔을 것들입니다.
...
## 2026-08-19 — 로컬 취재원 스윕 (클라우드 루틴 여드레째 0건)

### 취재원 조회 결과

| 취재원 | 결과 |
|---|---|
| CHOI (threads `@choi.openai`) | ✅ 글 5건 |
| Simon Willison | ✅ 최근 5건 (08-16 ~ 08-18) |
| Lilian Weng | — 최신이 **2026-07-04** (`harness-engineering` 에 이미 사용) |
| Ethan Mollick | — 최신이 **2026-07-23** |

블로그 취재원 둘은 **새 글이 없어서** 0건입니다. 조회 실패가 아닙니다 — 둘을 구분해 기록합니다.

### 글감 승격 → 발행

**Qwen 3.8 27B** (`qwen-38-27b`). **Willison 이 이틀에 걸쳐 두 번 쓴 것**이 신호였습니다.

1차 출처: [공식 모델 카드](https://huggingface.co/Qwen/Qwen3.8-27B) + [Artificial Analysis](https://artificialanalysis.ai/models/qwen3-8-27b).

- Intelligence Index **52** — GPT-5.6 Luna(max)와 동점, GLM-5.2(753B)·DeepSeek V4 Pro(1.7T)에 1점 차. 오픈 웨이트 동급 135개 중 1위, 동급 중앙값 9
- Apache 2.0 · 27B · 네이티브 비전 · 컨텍스트 262,144(1M 확장) · 4비트 17GB
- ⚠️ 추론 강도 기본값 `xhigh` — 펠리컨 SVG **21분 / 추론 22,276토큰(생성분 87%)** vs 추론 끔 **137초**(9.2배)
- Qwen 모델 카드는 **멀티턴 에이전트 기준으로 그 기본값을 옹호**함 → 대립이 아니라 "상정한 사용자가 다르다"로 정리

### 포착만 하고 큐로 넘긴 것 (전부 미검증)

| 리드 | 큐 | 다음에 확인할 것 |
|---|---|---|
| 404 Media — 희귀본 파쇄 스캔 → Amazon AI 학습 | `#42` | 탐사보도라 404 Media 자체가 1차. 저작권 쟁점 단정 금지 |
| GPT-5.6 비용 최적화 가이드 (18배, $33→$1.3) | `#43` | openai.com 403 → `learn.chatgpt.com`·쿡북 도메인 먼저 |
| Mojo 오픈소스화 (Apache 2) | `#44` | "Python 상위집합 포기" 서술의 근거 |
| DFlash 2 — 맥북 27B 70 tok/s, 4.6배 | `#45` | Inco AI 1차 발표 |
| Claude Cowork 모바일·웹 확장 | `#46` | ⚠️ 쓰기 전 `claude-api` 스킬 (절대 규칙 7) |

**제외한 것**: Max Tegmark "자살 경주" 발언 — 논평이라 이 블로그 톤과 거리가 있습니다. 인물 발언 자체가 사실인지와 별개로, 검증할 1차 사실이 없는 종류입니다.

### 교훈

**같은 사람이 같은 주제를 반복해서 쓰면 그게 신호입니다.** Willison 이 이틀에 두 번 쓴 게 이번 회차에서 가장 값어치 있는 단서였습니다. 취재원을 훑을 때 "새 글이 몇 건인가"보다 **"무엇이 반복되는가"** 를 봐야 합니다.

**블로그 취재원의 0건과 조회 실패는 다릅니다.** Weng·Mollick 은 7월 이후 새 글이 없습니다. 클라우드 루틴 로그의 "0건"은 차단 때문이지만 이건 그냥 안 쓴 것이라, 로그에서 구분해두지 않으면 나중에 오진합니다.

**자극적인 문장에는 단서를 붙여 씁니다.** "파라미터 63배 차이에 1점"이 이번 글에서 가장 눈에 띄는 대목인데, 비교 대상이 MoE 라 전체 파라미터 기준 비교는 과장됩니다. 그 문장을 쓰기로 했으면 단서도 같이 써야 합니다.
